...
//
// The write is atomic: the content is rendered in full before any file is
// touched, then written to a temporary file and renamed over the target. The
// callers pass the Go toolchain's own -importcfg path, so a partial write here
// would leave the toolchain with a truncated file it still depends on, and the
// original contents could not be recovered.
