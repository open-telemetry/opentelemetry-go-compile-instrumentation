...
func syncDeps(ctx context.Context, modPaths map[string]bool, moduleDir string) error {
	...
	for m := range modPaths {
		if path, isEmbedded := strings.CutPrefix(m, util.OtelcPkgRoot+"/"); isEmbedded {
	...
	modules[util.OtelcPkgRoot+"/instrumentation/shared"] = filepath.Join(
		util.GetBuildTempDir(), unzippedPkgDir,
