...
//   - every other segment must be a valid path.Match pattern (this catches
//     malformed bracket expressions such as "[z-a]" or an unclosed "[").
//
// An empty target is left to the rule constructors to reject; ValidateTarget
// treats it as a (trivially valid) exact path.
