...
func TestWrapDeclValues_NoInitializer(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "wrap_expression requires an existing initializer")
}
...
func TestApplyDeclRule_WrapExpression_Success(t *testing.T) {
	...
	r := &rule.InstDeclRule{
		...
		Kind:           "var",
		Identifier:     "X",
		WrapExpression: &rule.WrapExpressionAdvice{Template: "double({{ . }})"},
	}
...
func TestApplyDeclRule_NoOp_BothEmpty(t *testing.T) {
	file := makeVarFile("X", &dst.BasicLit{Kind: token.INT, Value: "1"})
	r := &rule.InstDeclRule{
		InstBaseRule: rule.InstBaseRule{Name: "noop"},
		Identifier:   "X",
		// Value and WrapExpression both zero — should be a no-op
	}

	err := newTestPhase().applyDeclRule(context.Background(), r, file)

	require.NoError(t, err)
	// Declaration must be unchanged
	spec := file.Decls[0].(*dst.GenDecl).Specs[0].(*dst.ValueSpec)
