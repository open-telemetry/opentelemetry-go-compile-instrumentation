...
func (ip *InstrumentPhase) applyDeclRule(ctx context.Context, r *rule.InstDeclRule, root *dst.File) error {
	if r.Value == "" && r.WrapExpression == nil {
		return nil
	}
	...
	if r.WrapExpression != nil {
		if err := wrapDeclValues(spec, r.WrapExpression.Template); err != nil {
			return err
		}
...
// Returns an error if spec has no initializers, since wrap_expression requires
// an existing value to substitute into {{ . }}.
func wrapDeclValues(spec *dst.ValueSpec, templateStr string) error {
	if len(spec.Values) == 0 {
		return ex.Newf(
			"wrap_expression requires an existing initializer but the declaration has none; " +
				"see docs/rules.md",
		)
	}
	...
	if err != nil {
		return ex.Wrapf(err, "failed to compile wrap_expression template")
	}
