...
func TestUpdateImportConfig(t *testing.T) {
	...
	t.Run("preserves original backup", func(t *testing.T) {
		tempDir := t.TempDir()
		cfgPath := filepath.Join(tempDir, "importcfg")
		originalContent := "packagefile fmt=/path/to/fmt.a\n"
		err := os.WriteFile(cfgPath, []byte(originalContent), 0o644)
		require.NoError(t, err)

		// Create a backup with specific content
		backupPath := cfgPath + ".original"
		...
		// Try to update with a new import - this would create backup if it didn't exist
		// But since backup already exists, it should not be overwritten
		// Intentionally ignoring error to test backup preservation
		_ = ip.updateImportConfig(map[string]string{"context": "context"})
		// This will likely fail since context resolution requires actual Go environment,
		// but that's okay - we just want to verify backup isn't overwritten

		// Check that backup was not overwritten
		content, err := os.ReadFile(backupPath)
		require.NoError(t, err)
		assert.Equal(t, backupContent, string(content), "backup should not be overwritten")
	})
	...
	t.Run("atomic write", func(t *testing.T) {
		...
		// Try to update - will fail at resolve step, but temp file should be cleaned up
		_ = ip.updateImportConfig(map[string]string{"nonexistent": "nonexistent/package"})

		// Verify temp file doesn't exist
		tempPath := cfgPath + ".tmp"
		_, err = os.Stat(tempPath)
		assert.True(t, os.IsNotExist(err), "temp file should not exist after failed update")
