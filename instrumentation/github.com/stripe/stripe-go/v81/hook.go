...
// Package v81 provides compile-time OpenTelemetry instrumentation for
...
//	RawRequest ─────┘
//
// Hooking that one function covers the entire SDK surface: the client.API
// aggregate and the package-level resource helpers alike. It needs no hook
// generated per method, and does not break when Stripe adds a resource.
//
// v81 predates the v2 API surface: it has no v2 resource services and no
// V2RawError, so unlike the sibling v82 package this one classifies v1 errors
// only. See apiErrorFrom.
//
// # Span shape
//
...
const (
	instrumentationName = "go.opentelemetry.io/otelc/instrumentation/github.com/stripe/stripe-go/v81"
...
//
// v81 has only the v1 error shape; the sibling v82 package additionally handles
// V2RawError, which does not exist here.
