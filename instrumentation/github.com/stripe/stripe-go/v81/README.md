# stripe-go v81 instrumentation

Compile-time OpenTelemetry instrumentation for
[`github.com/stripe/stripe-go/v81`](https://github.com/stripe/stripe-go).
...
|-------|--------|-----------|--------|
| Backend request | `(*BackendImplementation).requestWithRetriesAndTelemetry` | `{METHOD} {url.template}` | `go.opentelemetry.io/otelc/instrumentation/github.com/stripe/stripe-go/v81` |
...
Hooking that one function covers the whole SDK: the `client.API` aggregate and
the package-level resource helpers alike. It does not need updating when Stripe
adds a resource.

v81 predates the v2 API surface. It has no v2 services and no `V2RawError`, so
unlike the sibling `v82` package this one classifies v1 errors only.
...
standard library and OpenTelemetry. `*stripe.Error` is converted to the
version-neutral `semconv.APIError` at the boundary. `semconv/` is byte-identical
to the v82 copy, and error classification is the only place the two majors
differ.

## Supported versions

- Module: `github.com/stripe/stripe-go/v81`
- Rules apply to **v81.0.0+**.

This is the previous major, kept under
[ADR-0004](../../../../../docs/adr/0004-instrumentation-ownership-and-compatibility.md)'s
last-two-majors policy. It is a copy of the sibling `v82` directory with the
import path re-pointed, and the chokepoint has the same signature in both.
Following the `openai-go/v2`, `v3` precedent, each major carries its own copy of
`semconv/` instead of importing a shared one, so a v81 user never pulls
stripe-go v82 into their build. Keep the two in sync when changing either.
...
- **The rule has no upper version bound.** The hooked function is unexported, so
  a rename in a future v81 minor would silently drop instrumentation rather than
...
```bash
# Unit
go test ./instrumentation/github.com/stripe/stripe-go/v81/...

# Integration (requires: make build)
go -C test test -tags=integration -run TestStripeClientV81 ./integration/
```

The integration app lives in `test/apps/stripeclientv81`. It exercises a POST, a
GET by ID, and a collection GET through the `client.API` aggregate against an
in-process mock Stripe API. All of them must arrive through the single hook.
