...
* Third-party modules can distribute their own `otelc`-compatible instrumentations without changes to the `otelc` repository.
* Instrumentation packages become regular Go module dependencies and participate naturally in `go mod tidy` and existing dependency management workflows.
