...
- `exclude_targets` (list of strings, optional): Import paths to skip after `target` matches this dependency. Each entry is an exact import path or a glob using the same syntax as `target`, including `$root` to skip the root module and its subpackages. Useful for call-site rules that should apply broadly but not inside helper packages that already instrument themselves (for example otelgrpc). Evaluated at setup match time only; `$root` entries are expanded to root-module globs before matching.
