...
test-integration: build build-demo
	...
	set -euo pipefail
	go -C "test" test -json -v -shuffle=on -timeout=30m -count=1 -tags integration ./integration/... 2>&1 | tee ./gotest-integration.log

.ONESHELL:
test-latestlibbuild: build ## Run LatestLibBuild tests
	@echo "Running LatestLibBuild tests..."
	set -euo pipefail
	go -C "test" test -json -v -shuffle=on -timeout=30m -count=1 -tags latestlibbuild ./latestlibbuild/... 2>&1 | tee ./gotest-latestlibbuild.log
...
test-latestlibrun: build ## Run LatestLibRun tests (bump apps to @latest then run integration suite)
	...
	set -euo pipefail
	go -C "test" test -json -v -shuffle=on -timeout=30m -count=1 -tags latestlibrun ./latestlibrun/... 2>&1 | tee ./gotest-latestlibrun.log
	$(MAKE) tidy/test-apps
...
test-integration/coverage: build build-demo
	...
	set -euo pipefail
	go -C "test" test -json -v -shuffle=on -timeout=30m -count=1 -tags integration ./integration/... -coverprofile=../coverage-integration.txt -covermode=atomic 2>&1 | tee ./gotest-integration.log
