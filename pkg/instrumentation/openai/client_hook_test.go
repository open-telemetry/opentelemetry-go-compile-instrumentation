...
func TestBeforeAfterEmbeddingNew(t *testing.T) {
	resetInit()
	t.Setenv("OTEL_GO_ENABLED_INSTRUMENTATIONS", "OPENAI")
	...
	beforeEmbeddingNew(ictx, &openaisdk.EmbeddingService{}, ctx, params)

	res := &openaisdk.CreateEmbeddingResponse{
		Model: "text-embedding-3-small",
		Usage: openaisdk.CreateEmbeddingResponseUsage{
			PromptTokens: 8,
		},
	}
	afterEmbeddingNew(ictx, res, nil)
	...
	assert.Equal(t, "openai", attrs["gen_ai.system"])
	assert.Equal(t, "embeddings", attrs["gen_ai.operation.name"])
	assert.Equal(t, "text-embedding-3-small", attrs["gen_ai.request.model"])
	assert.Equal(t, int64(8), attrs["gen_ai.usage.input_tokens"])
}
...
func TestBeforeAfterCompletionNew(t *testing.T) {
	resetInit()
	t.Setenv("OTEL_GO_ENABLED_INSTRUMENTATIONS", "OPENAI")
	...
	beforeCompletionNew(ictx, &openaisdk.CompletionService{}, ctx, params)

	res := &openaisdk.Completion{
		ID:    "cmpl-test456",
		Model: "gpt-3.5-turbo-instruct",
	...
	afterCompletionNew(ictx, res, nil)
	...
	assert.Equal(t, "openai", attrs["gen_ai.system"])
	assert.Equal(t, "text_completion", attrs["gen_ai.operation.name"])
	assert.Equal(t, "cmpl-test456", attrs["gen_ai.response.id"])
	assert.Equal(t, int64(50), attrs["gen_ai.usage.input_tokens"])
	assert.Equal(t, int64(100), attrs["gen_ai.usage.output_tokens"])
}
...
func TestChatCompletionNewStreaming_CreatesSpan(t *testing.T) {
	resetInit()
	t.Setenv("OTEL_GO_ENABLED_INSTRUMENTATIONS", "OPENAI")
	...
	beforeChatCompletionNewStreaming(ictx, &openaisdk.ChatCompletionService{}, ctx, params)
	afterChatCompletionNewStreaming(ictx, nil) // nil stream = error case

	spans := sr.Ended()
	require.Len(t, spans, 1)
	assert.Equal(t, "chat gpt-4", spans[0].Name())
	assert.Equal(t, codes.Error, spans[0].Status().Code)
}

func TestCompletionNewStreaming_CreatesSpan(t *testing.T) {
	resetInit()
	t.Setenv("OTEL_GO_ENABLED_INSTRUMENTATIONS", "OPENAI")
	...
	beforeCompletionNewStreaming(ictx, &openaisdk.CompletionService{}, ctx, params)
	afterCompletionNewStreaming(ictx, nil)

	spans := sr.Ended()
	require.Len(t, spans, 1)
	assert.Equal(t, "text_completion gpt-3.5-turbo-instruct", spans[0].Name())
	assert.Equal(t, codes.Error, spans[0].Status().Code)
}
